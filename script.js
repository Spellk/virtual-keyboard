  ...
  new (class KeyboardModel {
    ...
    mapDataToLayout = (arg) => {
      return this.data.map((element) => {
        if (element[arg]) {
          if (this.statusShift) {
            return element["shift" + arg] || element[arg];
          } else {
            return element[arg];
          }
        } else if (element.group === "service" && element.key) {
          return element.key;
        } else {
          return "";
        }
